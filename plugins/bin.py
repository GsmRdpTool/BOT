...
async def bin(Client, message):
    try:
        ...
        # Usar texto del mensaje o del reply
        if message.reply_to_message and message.reply_to_message.text:
            message_text = message.reply_to_message.text
        else:
            message_text = message.text

        # Log para debug: ¿qué es from_user?
        print(f"[DEBUG] message.from_user: {repr(message.from_user)}")
        if message.reply_to_message:
            print(f"[DEBUG] message.reply_to_message.from_user: {repr(message.reply_to_message.from_user)}")
        ...
        if int(bin_code[0]) in waste_cards:
...
<b>【〄】 Verificado por:</b> <b><a ref="tg://user?id={user_id}">{nombre}</a> - {rol}</b>
